/**
 * GoLiveOverlay — Internal operator launch ceremony
 *
 * This component is ONLY rendered when:
 *   - launched = false  (server-confirmed)
 *   - ?launchMode = true  (private operator URL)
 *
 * It is NEVER shown to normal public visitors.
 * LaunchGate controls which component is rendered based on server state.
 *
 * Flow:
 *   1. Auth screen — operator enters LAUNCH_SECRET via password input
 *   2. Cinematic entrance animation
 *   3. Operator clicks "LAUNCH Gremake"
 *   4. POST /api/launch → Authorization: Bearer <secret>
 *   5. On success: grand-opening celebration, then onLaunched() called
 *   6. On failure: clear error, operator can retry
 *
 * Props:
 *   onLaunched: () => void  — called after celebration completes
 *
 * Security:
 *   - Secret typed into <input type="password"> — never in URL
 *   - Secret held only in useRef<string> — not in React state
 *   - Secret cleared immediately after POST
 *   - Secret never in localStorage, console, or URL
 *
 * Dev reset: ?resetGoLive=true clears localStorage cache (does not affect DB)
 */
...
function applyDevReset(): void {
  try {
    ...
    if (url.searchParams.get("resetGoLive") === "true") {
      localStorage.removeItem("gremake_go_live_seen");
...
type Phase =
  | "auth"         // Password input screen
  | "intro"        // Cinematic entrance
  | "ready"        // CTA visible
  | "launching"    // POST in progress
  | "celebration"  // Grand opening animation
  | "exit";        // Fading out (then onLaunched called)
...
export default function GoLiveOverlay({ onLaunched }: GoLiveOverlayProps) {
  ...
  const handleLaunch = useCallback(async () => {
    ...
    secretRef.current = ""; // Clear immediately
    ...
    add(() => onLaunched(), reduced ? 2400 : 4800);
  }, [phase, isLaunching, ctaControls, reduced, onLaunched]);
  ...
  if (phase === "auth") {
    return (
      <div
        ...
        role="dialog" aria-modal="true" aria-label="Gremake Internal Launch Authentication"
      >
        <motion.div
        ...
        >
          ...
          <h1 className="mb-8 text-xl font-bold text-white tracking-tight">Gremake Grand Opening</h1>
          <form onSubmit={handleAuth} className="w-full" noValidate>
            <label htmlFor="launch-secret" className="sr-only">Launch Secret</label>
            <input
              id="launch-secret" ref={secretInputRef}
            ...
            />
            {authError && <p id="auth-error" className="mt-2 text-xs text-red-400/80">{authError}</p>}
            <button
              type="submit"
              ...
              style={{ background: `linear-gradient(135deg, ${ACCENT}cc, ${ACCENT})`, boxShadow: `0 0 0 1px ${ACCENT}33, 0 6px 20px ${ACCENT}28` }}
            >
              Continue
            </button>
          </form>
          <p className="mt-6 text-center text-xs text-white/20">Authorized personnel only</p>
        </motion.div>
      </div>
    );
  }

  // ── Cinematic overlay (intro → ready → launching → celebration → exit) ─────
  return (
    <>
      <ParticleCanvas active={showParticles} reduced={reduced} />

      <AnimatePresence>
        {phase !== "exit" || true /* keep mounted during exit animation */}
        <motion.div
          key="go-live-overlay"
          role="dialog" aria-modal="true" aria-label="Gremake Grand Opening"
          initial={{ opacity: 0 }}
        ...
        >
          ...
          <motion.div aria-hidden="true"
            ...
            animate={{ opacity: isCelebrating ? [0.18, 0.38, 0.22] : 0.12, scale: isCelebrating ? [1.0, 1.6, 1.2] : 1.0 }}
              ...
              {!isCelebrating && (
                ...
                >
                  ...
                  >
                    <motion.button
                      animate={ctaControls}
                      onClick={handleLaunch} onKeyDown={handleLaunchKey}
                      tabIndex={0} aria-label="Launch Gremake"
